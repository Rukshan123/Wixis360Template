...
The first step to migrate wixis360 v2.4.x to v3.0 is upgrade the Bootstrap 3 base code to Bootstrap 4, the full instruction <a href="https://getbootstrap.com/docs/4.3/migration/">here</a> after you upgraded the base code you need to update the markups.

#### CSS / JS Files
Since wixis360 v3.0 Bootstrap 4 is complete included in wixis360's CSS file. You will not need to load the Bootstrap CSS file `bootstrap(.min).css`, but you will need the Bootstrap JS file `bootstrap(.min).js`.
...
2. Logo / Brand Link
  - `<a href="index3.html" class="brand-link"><img src="dist/img/wixis360Logo.png" alt="wixis360 Logo" class="brand-image img-circle elevation-3" style="opacity: .8"><span class="brand-text font-weight-light">wixis360 3</span></a>` replaces the old logo
3. Sidebar
...
##### Content Header
The biggest change in content header is wixis360 use here now `.container-fluid`, `.row` & `.col-*` and the breadcrumb markup changed, here are all changes:
